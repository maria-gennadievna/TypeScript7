let skyButton = document.getElementById('sky')
if (skyButton) skyButton.onclick = () => {
    sky(1000)
}


function sky(starQty: number) {
    let i = 1
    while (i < starQty) {
        let starSize = Math.random() * 5
        let positionTop = Math.random() * window.innerHeight
        let positionLeft = Math.random() * window.innerWidth
...
function star(size: number, position: { top: string, left: string }) {
...
<path style="fill:#ED8A19;" d="M26.285,2.486l5.407,10.956c0.376,0.762,1.103,1.29,1.944,1.412l12.091,1.757
    ...
    if (wrapper) wrapper.appendChild(starElement)
}
