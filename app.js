...
function star(size, position) {
    ...
    starElement.innerHTML = "<svg id=\"star\"  version=\"1.1\" id=\"Capa_1\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" \n\t viewBox=\"0 0 47.94 47.94\" xml:space=\"preserve\">\n<path  d=\"M26.285,2.486l5.407,10.956c0.376,0.762,1.103,1.29,1.944,1.412l12.091,1.757\n\tc2.118,0.308,2.963,2.91,1.431,4.403l-8.749,8.528c-0.608,0.593-0.886,1.448-0.742,2.285l2.065,12.042\n\tc0.362,2.109-1.852,3.717-3.746,2.722l-10.814-5.685c-0.752-0.395-1.651-0.395-2.403,0l-10.814,5.685\n\tc-1.894,0.996-4.108-0.613-3.746-2.722l2.065-12.042c0.144-0.837-0.134-1.692-0.742-2.285l-8.749-8.528\n\tc-1.532-1.494-0.687-4.096,1.431-4.403l12.091-1.757c0.841-0.122,1.568-0.65,1.944-1.412l5.407-10.956\n\tC22.602,0.567,25.338,0.567,26.285,2.486z\"/>\n</svg>";
    starElement.style.width = Math.floor(Math.random() * 10) + 1 + 'px';
    starElement.style.height = Math.floor(Math.random() * 40) + 1 + 'px';
    starElement.style.fill = 'rgb(' + (Math.floor(Math.random() * 256)) + ',' + (Math.floor(Math.random() * 256)) + ',' + (Math.floor(Math.random() * 256)) + ')';
...
async function Draw(){
  ...
  if(!t){
    ...
    lineFaceI = (X1, Y1, Z1, X2, Y2, Z2, facet, autoFlipNormals=true, showNormals=false) => {
    ...
    iPv = .1
    ls = 5
    bounding = subDividedTetrahedron(ls/2,4,2.5).map(v=>{
      return [v,0]
    })
    P = Array(iPc).fill().map(v=>{
      X = S(p=Math.PI*2*Rn()) * S(q=Math.PI/2) * ls/20
      Y = 0//C(q) * ls/2
      Z = C(p) * S(q) * ls/20
      v = iPv//.5*iPv + Rn()*iPv*.5
      vx = S(p) * S(q=Rn()<.5?Math.PI/2*Rn()**.5:Math.PI-Math.PI/2*Rn()**.5) * v
      vy = C(q) * v
      vz = C(p) * S(q) * v
  ...
  oX=0, oY=0, oZ=Math.min(8,Math.max(2.5,(.3+C(t/2))*20))
  Rl=0, Pt=S(t/2)/2-.2, Yw=t/3
  ...
  bounding.map(v=>{
    ...
    col1 = '#ff000010'
    col2 = `hsla(${50-v[1]**.5*200},99%,${50+v[1]*60}%,${alpha = Math.min(.75, (v[1]/=1.066)*2)})`
    stroke(col1, alpha>.05?col2:'#ff000010', 1, true)
  })
  ...
  P.map((v, i) => {
    ...
    P.map((q, j)=>{
      if(i!=j){
        ...
        Y2 = q[1]
        Z2 = q[2]
        d__ = Math.hypot(X2-X1,Y2-Y1,Z2-Z1)
        if(d__<5){
          x.beginPath()
          X = X1
          Y = Y1
          ...
          x.globalAlpha = 1
          lw = 4/(1+(1+d__)**2*1)
          if(lw>.5) stroke(`hsla(${d__*160},99%,${Math.max(50,120-(1+d__)**2*8)}%,${Math.min(.5, lw/3)})`,'',lw,true)
        }
      }
    })
    X = X1
    Y = Y1
    Z = Z1
    if(Math.hypot(X,Y,Z)>ls*2){
      X = v[0] = 0
      Y = v[1] = 0
      Z = v[2] = 0
    }
    d_ = Math.hypot(v[3],v[4],v[5])
    dx = v[3] / d_ * iPv
    dy = v[4] / d_ * iPv
    dz = v[5] / d_ * iPv
    v[3] += (dx-v[3])/20
    ...
    if(Z>0){
      ...
      bounding.map(q => {
        ...
        ay /= q[0].length
        az /= q[0].length
        d = Math.hypot(ax-X1,ay-Y1,az-Z1)
        if(d<.66 && (l=lineFaceI(X1,Y1,Z1,X2,Y2,Z2,q[0]))){
          rx = X1 - l[0][0]
          ry = Y1 - l[0][1]
          rz = Z1 - l[0][2]
          ...
          ry /= d
          rz /= d
          v[3] += l[1][0] * d_ * rebound
          v[4] += l[1][1] * d_ * rebound
          v[5] += l[1][2] * d_ * rebound
          q[1]+=1
          spawnSparks(...l[0])
        }
      })
